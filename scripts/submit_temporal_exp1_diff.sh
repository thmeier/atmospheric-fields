#!/bin/bash
#SBATCH --job-name=tmp_exp1_diff
#SBATCH --time=24:00:00
#SBATCH --account=pmlr_jobs
#SBATCH --mem=48G
#SBATCH --gpus=1
#SBATCH --constraint=2080ti
#SBATCH --output=logs/tmp_exp1_diff_%j.out
#SBATCH --error=logs/tmp_exp1_diff_%j.err

# Experiment 1: pure difference field X_diff = X_t − X_{t−24h}  (4 channels)
# Forces the encoder to learn the temporal derivative — kills the dominant
# macroscopic-state variance that washes out static-encoder PCAs.
...
OUTPUT_DIR="/work/scratch/${USER}/results/may_13_temporal_exp1_diff_d512_maxpool"
MODEL_SIZE="twin"
EMBED_DIM=512
EPOCHS=150
BATCH=64
...
echo "Starting temporal EXP1 (mode=$TEMPORAL_MODE Δt=${DELTA_HOURS}h) on: $(hostname)"
