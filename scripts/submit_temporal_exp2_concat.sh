#!/bin/bash
#SBATCH --job-name=tmp_exp2_concat
#SBATCH --time=24:00:00
#SBATCH --account=pmlr_jobs
#SBATCH --mem=48G
#SBATCH --gpus=1
#SBATCH --constraint=2080ti
#SBATCH --output=logs/tmp_exp2_concat_%j.out
#SBATCH --error=logs/tmp_exp2_concat_%j.err

# Experiment 2: state concatenation X_concat = [X_{t−24h}, X_t]  (8 channels)
# Gives the network both absolute state and temporal context — needs the
# end-of-training PatchEmbed weight-norm diagnostic to confirm the prior
# channels aren't being ignored.
...
OUTPUT_DIR="/work/scratch/${USER}/results/may_13_temporal_exp2_concat_d512_maxpool"
MODEL_SIZE="twin"
EMBED_DIM=512
EPOCHS=150
BATCH=64
...
N_SAMPLES=500
TEMPORAL_MODE="concat"
DELTA_HOURS=24

mkdir -p "$OUTPUT_DIR"

echo "Starting temporal EXP2 (mode=$TEMPORAL_MODE Δt=${DELTA_HOURS}h) on: $(hostname)"
